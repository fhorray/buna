...
| `buna build --runtime <bun\|node\|cloudflare>` | Runs the full build pipeline (packages + apps). When `cloudflare` is selected the worker bundle is written to `.buna/cloudflare`. |
...
Running `buna build --runtime cloudflare` creates the worker payload at `.buna/cloudflare/` with the following files:
...
* Worker artifacts are located in `.buna/cloudflare`. Run `buna build --runtime cloudflare` to regenerate them whenever routes or assets change.
