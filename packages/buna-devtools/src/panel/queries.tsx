...
import { invalidateQuery, revalidateQuery } from '@buna/router';
import {
  baseCardClass,
  monoClass,
  badgeClass,
  ...
  return (
    <section class={`${baseCardClass} flex-1 flex flex-col min-h-0`}>
      <div class="mb-2 flex items-center justify-between gap-2">
        <span class="text-[11px] font-semibold uppercase tracking-[0.08em] text-blue-300">
          Queries
        </span>
      ...
      <div class="flex flex-wrap gap-1.5 mb-2 items-center">
        <input
          ...
          class="flex-1 min-w-[120px] rounded-full border border-slate-700 bg-slate-950 px-2.5 py-1.5 text-[11px] text-slate-100 outline-none placeholder:text-slate-500 focus:border-blue-500/50 transition-colors"
        />
              ...
              >
                ...
                >
                  <div class="flex flex-col min-w-0">
                    <span
                      class={`${monoClass} text-[11px] uppercase tracking-[0.08em] font-bold text-blue-400`}
                    >
                      {String(method)}
                    </span>
                    <span
                  ...
                  <div class="flex flex-col items-end gap-1">
                    <div class="flex items-center gap-1.5">
                      <span
                        class={`${badgeClass} ${getStatusClasses(q.status)}`}
                      >
                        {status}
                      </span>
                    ...
                    <div class="flex gap-1.5">
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          invalidateQuery(q.key as any[]);
                        }}
                        class="px-2 py-0.5 rounded-full border text-[10px] border-slate-600 hover:bg-slate-800 transition-colors"
                      >
                        Invalidate
                      </button>
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          revalidateQuery(q.key as any[]);
                        }}
                        class="px-2 py-0.5 rounded-full border text-[10px] border-blue-500/70 bg-blue-500/10 hover:bg-blue-500/20 transition-colors"
                      >
                        Refetch
                      </button>
                    </div>
                ...
                {isExpanded && (
                  <div class="mt-2 pt-2 border-t border-slate-700 flex flex-col gap-2">
                    <div>
                      ...
                      <pre
                        class={`${monoClass} bg-slate-950 rounded-lg p-2 border border-slate-700 max-h-40 overflow-auto text-slate-100`}
                      >
                        {q.data == null
                          ? 'null'
                    ...
                    {q.error && (
                      <div>
                        ...
                        <pre
                          class={`${monoClass} bg-slate-900 rounded-lg p-2 border border-rose-900/80 max-h-40 overflow-auto text-rose-100`}
                        >
