...
function setNamedMeta(name: string, content: string | undefined) {
  ...
  let tag = document.head.querySelector<HTMLMetaElement>(
    `meta[name="${name}"][data-buna-meta="true"]`,
  );
  if (!tag) {
...
function setPropertyMeta(property: string, content: string | undefined) {
  ...
  let tag = document.head.querySelector<HTMLMetaElement>(
    `meta[property="${property}"][data-buna-meta="true"]`,
  );
  if (!tag) {
    tag = document.createElement("meta");
...
function setLink(
  ...
  let tag = document.head.querySelector<HTMLLinkElement>(
    `link[rel="${rel}"][href="${href}"][data-buna-meta="true"]`,
  );
  if (!tag) {
...
/**
 * Remove all previous Buna-managed tags before applying new ones.
 */
...
function applyBunaMeta(meta?: BunaMeta) {
  ...
  // Clear previous Buna-managed tags to avoid duplicates
  ...
  if (meta.charset) {
    let charsetTag = document.head.querySelector<HTMLMetaElement>(
      'meta[charset][data-buna-meta="true"]',
    );
    if (!charsetTag) {
